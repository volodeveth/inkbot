...
│   │   └── screenshots.tsx.bak        🔒 Screenshot generator (вимкнено, перейменовано в .bak, використовує реальні скріншоти додатку)
│   │
│   ├── components/
...
- `/screenshots` — Screenshot generator для App Store (вимкнено, файл `screenshots.tsx.bak`; для повернення — перейменувати назад у `.tsx`)
...
### Screenshots Page (`screenshots.tsx.bak`)
Сторінка для створення App Store скріншотів. Зараз вимкнена (перейменовано в `.bak`).
Для повернення: перейменувати `app/routes/screenshots.tsx.bak` → `screenshots.tsx`.
